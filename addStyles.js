const bodyClass = "bg-purple-800 bg-gradient-to-br from-blue-200 bg-opacity-25";
const bdy = document.querySelector("body");
bodyClass.split(" ").forEach((csl) => bdy.classList.add(cls));
...
function addClasses(button) {
    ...
    const btnDecimalClass = "hover:bg-yelllow-500 border-yellow-500";
    ...
    const baseClass = "number flex w=12 h-12 justify-center items-center m-1 font-bold py-2 px-2 border hover:border-transparent rounded text-white border-2xl";
    baseClass.split(" ").forEach((cls) => base.classList.add(cls));
}
